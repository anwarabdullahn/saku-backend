...
module.exports.Store = (req, res) => {
	const { walletId, categoryId, type, desc, amount, date } = req.body,
...
module.exports.Get = (req, res) => {
	...
	walletId
		? Transaction.find({ walletId }).sort({ date: 'desc' }).then((transaction, err) => {
				err && res.json(err);
				transaction
					? res.status(200).json({
							success: true,
...
module.exports.Edit = (req, res) => {
	...
	Wallet.findById(walletId).then((wallet, err) => {
		if (err) res.json(err);
		wallet
			? Transaction.findOne({ walletId, _id: id }).then((transaction, err) => {
					if (!transaction) {
					...
					transaction.type == 'Plus'
						? (balance = parseInt(balance) - parseInt(trbalance))
						: (balance = parseInt(balance) + parseInt(trbalance));

					wallet.balance = balance;

					toBeTransaction.type == 'Plus'
						? (wallet.balance = parseInt(wallet.balance) + parseInt(toBeTransaction.amount))
						: (wallet.balance = parseInt(wallet.balance) - parseInt(toBeTransaction.amount));
	...
	Wallet.findById(walletId).then((wallet, err) => {
		...
		wallet
			? Transaction.findOne({ walletId, _id: id }).then((transaction, err) => {
					...
					wallet.save().then(() => {
						Transaction.findOneAndRemove({ walletId, _id: id }).then((data) => {
							res.status(200).json({
								success: true,
